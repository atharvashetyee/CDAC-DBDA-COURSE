...
select region_name,
replace(region_name,Europe,EU) as r
from regions;
...
select concat(first_name,"-",last_name,"-",job_id) as con
from employees;
...
substring(trim(city),1,3) as sub
from locations;
...
select last_name,
position("n" in last_name) as pos
from employees
...
select job_title,
locate("e",job_title) as pos
from jobs
where locate("e",job_title)>5;
...
select country_name,
left(country_name,2) as first_part,
right(country_name,2) as last_part
from countries;
 ...
 select last_name
 from employees
 where position("o" in last_name)>length(last_name)/2;
...
select department_id,count(employee_id) as c_emp
from departments 
where department_id in(50,60,70)
...
select concat(first_name," ",last_name," ",country_name) as single
from employees e
join departments d
